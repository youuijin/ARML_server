...
from trades import trades_loss_meta
...
class Meta(nn.Module):
    ...
    def forward_trades(self, x_spt, y_spt, x_qry, y_qry):
        """
        :param x_spt:   [b, setsz, c_, h, w]
        :param y_spt:   [b, setsz]
        :param x_qry:   [b, querysz, c_, h, w]
        ...
        for i in range(task_num):
            ...
            for k in range(1, self.update_step):
                ...
                if k==self.update_step - 1:
                    ...
                    logits_q = self.net(x_qry[i], fast_weights, bn_training=True)
                    logits_q_adv = self.at.perturb(fast_weights, x_qry[i], y_qry[i])
                    loss_clean = F.cross_entropy(logits_q, y_qry[i])
                    loss_adv = (1.0/len(x_qry[i]))*criterion_kl(torch.log(F.softmax(logits_q_adv, dim=1) + epsilon), torch.log(F.softmax(logits_q, dim=1) + epsilon))

                    loss = loss_clean + beta*loss_adv
                    
                    # if loss_adv.isnan().any():
                    #     print("logits_q : ", logits_q[0])
                    #     print("logits_q_Adv : ", logits_q_adv[0])

                    #     print("loss_clean : ", loss_clean.isnan().any())
                    #     print("loss_adv : ", loss_adv.isnan().any())

                    #     print("log_softmax : ", torch.log(F.softmax(logits_q_adv, dim=1) + epsilon)[0])
                    #     print("softmax : ", torch.log(F.softmax(logits_q, dim=1) + epsilon)[0])
                    ...
                    optimizer.zero_grad()
                    # loss, loss_clean, loss_adv = trades_loss_meta(self.net, fast_weights, x_qry[i], y_qry[i], self.at)
                    pred_q = F.softmax(logits_q, dim=1).argmax(dim=1)
                    correct = torch.eq(pred_q, y_qry[i]).sum().item()  # convert to numpy
                    corrects  += correct
                    losses_clean += loss_clean
                    losses += loss
                    ...
                    if need_adv:
                        ...
                        pred_q_adv = F.softmax(logits_q_adv, dim=1).argmax(dim=1)
                        correct_adv = torch.eq(pred_q_adv, y_qry[i]).sum().item()
                        corrects_adv = corrects_adv + correct_adv
                        losses_adv += loss_adv

        # end of all tasks
        # sum over all losses on query set across all tasks
        loss_q = losses / task_num #마지막 원소 사용 -> K step update한 후!
        loss_q_clean = losses_clean / task_num
        loss_q_adv = losses_adv / task_num
        ...
        return accs, accs_adv, loss_q, loss_q_clean, loss_q_adv
    ...
    def set_test_attack(self, attack, eps=6, iter=10):
        if attack=="Auto Attack":
            self.test_at = AutoAttack(self.net, norm='Linf', eps=eps/255, version='standard')
        else:
